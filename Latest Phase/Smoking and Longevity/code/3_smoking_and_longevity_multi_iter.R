...
#   Latest Phase/Smoking and Longevity/results
# ============================================================
...
run_one_iteration = function(iter_seed,
  ...
  b_x = 18 +
    ...
    2.0 * x4_lowlit_s +
    4.0 * x5_s +
    1.5 * pmax(x1_s, 0) * pmax(x3_poorlung_s, 0)
  ...
  eval_one = function(est, name) {
    ...
    pred_ate = mean(est, na.rm = TRUE)
    true_ate_test = mean(tau_test, na.rm = TRUE)
    ...
    data.frame(
      ...
      pred_ate = round(pred_ate, 4),
      true_ate_test = round(true_ate_test, 4),
      stringsAsFactors = FALSE
    )
  }
  ...
  iter_rows$true_ate_full    = round(mean(tau_x), 4)
...
summarise_learner = function(nm) {
  ...
  raw_rmse = mean_ci(d$raw_rmse)
  mse = mean_ci(d$norm_mse)
  rmse     = mean_ci(d$norm_rmse)
  rc  = mean_ci(d$rank_corr)
 
  r1  = mean_ci(d$rec_sg1)
  ...
  pate = mean_ci(d$pred_ate)
  tate = mean_ci(d$true_ate_test)
  ...
  data.frame(
    ...
    median_raw_mse = median_safe(d$raw_mse),
    
    mean_raw_rmse = raw_rmse$mean,
    sd_raw_rmse   = raw_rmse$sd,
    ci_lo_raw_rmse = raw_rmse$ci_lo,
    ci_hi_raw_rmse = raw_rmse$ci_hi,
    median_raw_rmse = median_safe(d$raw_rmse),

    mean_norm_mse = mse$mean,
    sd_norm_mse   = mse$sd,
    ci_lo_norm_mse = mse$ci_lo,
    ci_hi_norm_mse = mse$ci_hi,
    median_norm_mse = median_safe(d$norm_mse),
    
    mean_norm_rmse = rmse$mean,
    sd_norm_rmse   = rmse$sd,
    ci_lo_norm_rmse = rmse$ci_lo,
    ci_hi_norm_rmse = rmse$ci_hi,
    median_norm_rmse = median_safe(d$norm_rmse),
    ...
    mean_rec_sg1 = r1$mean,
    ci_lo_rec_sg1 = r1$ci_lo,
    ci_hi_rec_sg1 = r1$ci_hi,
    median_rec_sg1 = median_safe(d$rec_sg1),
    
    mean_rec_sg2 = r2$mean,
    ci_lo_rec_sg2 = r2$ci_lo,
    ci_hi_rec_sg2 = r2$ci_hi,
    median_rec_sg2 = median_safe(d$rec_sg2),

    mean_rec_sg3 = r3$mean,
    ci_lo_rec_sg3 = r3$ci_lo,
    ci_hi_rec_sg3 = r3$ci_hi,
    median_rec_sg3 = median_safe(d$rec_sg3),
    ...
    median_sg3_inflated = median_safe(as.numeric(d$sg3_inflated)),
    
    mean_pred_ate = pate$mean,
    ci_lo_pred_ate = pate$ci_lo,
    ci_hi_pred_ate = pate$ci_hi,
    median_pred_ate = median_safe(d$pred_ate),
    
    mean_true_ate_test = tate$mean,
    ci_lo_true_ate_test = tate$ci_lo,
    ci_hi_true_ate_test = tate$ci_hi,
    median_true_ate_test = median_safe(d$true_ate_test),
