...
cat("Mean Y (all patients):", round(mean(y), 2), "\n")
...
#
# SG1 — lower-risk subgroup, should still be harmed but less so
#   younger, high income, low stress, high literacy
#
# SG2 — clinically vulnerable subgroup, should be most harmed
#   older, poor baseline lungs, high stress
#
# SG3 — socially disadvantaged subgroup
#   low income and low literacy
# ============================================================

sg1 = x1_age < median(x1_age) &
  x2_income > median(x2_income) &
  x5_stress < median(x5_stress) &
  x4_literacy > median(x4_literacy)

sg2 = x1_age > quantile(x1_age, 0.75) &
  x3_lung < quantile(x3_lung, 0.25) &
  x5_stress > quantile(x5_stress, 0.75)

sg3 = x2_income < quantile(x2_income, 0.25) &
  x4_literacy < quantile(x4_literacy, 0.25)

cat("\n=== SUBGROUP SIZES ===\n")
cat("Subgroup 1 (younger, high-income, low-stress, literate):", sum(sg1), "\n")
